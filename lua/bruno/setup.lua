...
function M._verify_prereqs()
    vim.schedule(function()
        ...
        M._check_cli_prereq("!tree-sitter")
...
function M.debug_state()
    ...
    M._print("repo_dir: " .. M.local_treesitter_repo_dir)
...
function M._sync_repo()
    M.utils.Syscall({ "mkdir", "-p", M.local_treesitter_repo_dir }, {}, M.on_exit)
    if vim.fn.isdirectory(M.local_treesitter_repo_dir .. "/.git") == 0 then
        M._print("cloning " .. M.treesitter_url .. " to " .. M.local_treesitter_repo_dir)
        M.utils.Syscall({ "git", "clone", "--depth", "1", M.treesitter_url, "." }, { cwd = M.local_treesitter_repo_dir },
            M.on_exit)
    else
        M._print("pulling from " .. M.treesitter_url)
        M.utils.Syscall({ "git", "-C", M.local_treesitter_repo_dir, "pull" }, { cwd = M.local_treesitter_repo_dir }, M
            .on_exit)
    end
    if vim.fn.isdirectory(M.local_treesitter_repo_dir .. "/queries") == 0 then
        error("queries directory not found in " .. M.local_treesitter_repo_dir)
    end
    M.utils.Syscall({ "tree-sitter", "generate" }, { cwd = M.local_treesitter_repo_dir }, M.on_exit)
    M.utils.Syscall({ "tree-sitter", "test" }, { cwd = M.local_treesitter_repo_dir }, M.on_exit)
end
...
function M._setup_queries()
    ...
    local src_queries_dir = M.local_treesitter_repo_dir .. "/queries"
...
function M._setup_treesitter()
    ...
    parser_configs.bruno = {
        install_info = {
            url = M.local_treesitter_repo_dir,
...
M.teardown = function()
    if not M.local_treesitter_repo_dir or M.local_treesitter_repo_dir == "" then
    ...
    if vim.fn.isdirectory(M.local_treesitter_repo_dir) == 1 then
        M.utils.Syscall({ "rm", "-rf", M.local_treesitter_repo_dir }, {}, M.on_exit)
    end
